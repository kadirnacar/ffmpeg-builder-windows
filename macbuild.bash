...
# git clone --depth 1 -b n4.4.4 https://github.com/FFmpeg/FFmpeg $CMPLD/ffmpeg &
# git clone --depth 1 -b v2.0.1 https://aomedia.googlesource.com/aom.git $CMPLD/aom &
# wait

# cd $CMPLD/ffmpeg 
# git apply ../../../transition.patch 
# cd .. 
# cd .. 
# cp ../gltransition/vf_gltransition.c $CMPLD/ffmpeg/libavfilter 
# cp ../opengl/vf_genericshader.c $CMPLD/ffmpeg/libavfilter 

# curl -Ls -o - https://ftp.osuosl.org/pub/blfs/conglomeration/x265/x265_3.3.tar.gz | tar zxf - -C $CMPLD/ &
...
# {(curl -Ls -o - https://github.com/harfbuzz/harfbuzz/releases/download/7.2.0/harfbuzz-7.2.0.tar.xz | tar Jxf - -C $CMPLD/) &};
...
function build_gettext () {
  if [[ ! -e "${SRC}/lib/libgettextlib.la" ]]; then
...
function build_harfbuzz () {
  if [[ ! -e "${SRC}/lib/pkgconfig/harfbuzz.pc" ]]; then
    ...
    cd ${CMPLD}
    cd harfbuzz-7.2.0
    ./configure --prefix=${SRC} --disable-shared --enable-static --with-freetype=yes --with-fontconfig=no --with-icu=no
    make -j ${NUM_PARALLEL_BUILDS}
    make install
  fi
}
...
function build_ffmpeg () {
  ...
  export LDFLAGS="$LDFLAGS -lexpat -lenca -lfribidi -liconv -lstdc++ -lfreetype -lglfw -framework CoreText -framework VideoToolbox -framework OpenGL"
  ./configure --prefix=${SRC} --extra-cflags="-fno-stack-check" --arch=${ARCH} --cc=/usr/bin/clang \
              ...
              --disable-ffplay --enable-postproc --enable-nonfree --enable-runtime-cpudetect  --enable-opengl --enable-filter=gltransition --enable-filter=genericshader
  echo "build start"
  start_time="$(date -u +%s)"
  make -j ${NUM_PARALLEL_BUILDS}
  end_time="$(date -u +%s)"
