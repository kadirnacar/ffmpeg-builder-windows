...
build_glew() {
  ...
  cd glew-2.2.0/build
    local cmake_params="-DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=ON"
    if [[ $compiler_flavors != "native" ]]; then
    ...
    do_make_and_make_install
  cd ../..
  sed -i.bak 's/-lglew32.*/-lglew32 -lglew/' "$PKG_CONFIG_PATH/glew.pc"
  cp "$PKG_CONFIG_PATH/lglew.pc" "$PKG_CONFIG_PATH/glew32.pc"
}

build_glfw() {
  download_and_unpack_file https://github.com/glfw/glfw/releases/download/3.3.2/glfw-3.3.2.zip glfw-3.3.2
  cd glfw-3.3.2
    do_cmake_and_install "-DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=ON"
  cd ..
}
...
build_libgme() {
  ...
  download_and_unpack_file https://src.fedoraproject.org/repo/pkgs/game-music-emu/game-music-emu-0.6.3.tar.xz/sha512/4b20c69ced696bb879c34bcb7ce0f5f276642458d4cebca8ede673eed7d50664e527626e2077f85a3411a26660f1b3f01e43cccd72945e1edb2994421efeb552/game-music-emu-0.6.3.tar.xz
...
build_ffmpeg() {
  ...
  cd $output_dir
    ...
    init_options="--pkg-config=pkg-config --pkg-config-flags=--static --extra-version=kadirnacar --enable-version3 --disable-debug --disable-w32threads"
    if [[ $compiler_flavors != "native" ]]; then
    ...
    config_options+=" --extra-libs=-lglew32" 
    config_options+=" --extra-libs=-lglfw3" 
    ...
    config_options+=" --enable-filter=gltransition"
    config_options+=" --enable-filter=genericshader"
